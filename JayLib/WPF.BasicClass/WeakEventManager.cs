...
namespace JayLib.WPF.BasicClass
{
    ...
    public class WeakEventManager
    {
        ...
        {
            var toRaiseSubscriber = new List<object>();
            var toRaiseHandler = new List<MethodInfo>();
            ...
            if (eventHandlers.TryGetValue(eventName, out var target))
            {
                for (var i = 0; i < target.Count; i++)
                {
                    ...
                    if (isStatic)
                    {
                        ...
                        toRaiseSubscriber.Add(null);
                        toRaiseHandler.Add(subscription.Handler);
                        continue;
                    }
            ...
            for (var i = 0; i < toRaiseSubscriber.Count && i < toRaiseHandler.Count; i++)
            {
                //(var subscriber, var handler) = toRaise[i];
                var subscriber = toRaiseSubscriber[i];
                var handler = toRaiseHandler[i];
