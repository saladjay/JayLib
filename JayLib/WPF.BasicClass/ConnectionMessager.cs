...
{
    public class ConnectionMessager
    {
        #region Constructor

        public ConnectionMessager()
        {
        }
        ...
        public void Register(int? app, int? device, int? dataType, Action<byte[]> callback)
        {
            bool isBroadMatch = StringReplace(out string message, app, device, dataType);
            this.Register(message, callback, typeof(byte[]), isBroadMatch);
        }

        private bool StringReplace(out string message, params int?[] value)
        {
            message = "";
            bool res = false;
            foreach (var item in value)
            {
                ...
                {
                    res = true;
                    message = message.Append("\\w{4}");
                }
                else
                {
                    int data = (int)item;
                    message = message.Append(data.ToString("x4"));
                }
            }
            if (res)
            {
                message = message.PreAppend("^").Append("$");
            }
            return res;
        }
        /// <summary>
        ...
        void Register(string message, Delegate callback, Type parameterType, bool isBroadMatch = false)
        {
            ...
            _messageToActionsMap.AddAction(message, callback.Target, callback.Method, parameterType, isBroadMatch);
        }
        ...
        public void Notify(int app, int device, int dataType, byte[] parameter)
        {
            bool isBroadMatch = StringReplace(out string message, app, device, dataType);
        ...
        private class MessageToActionsMap
        {
            ...
            internal void AddAction(string message, object target, MethodInfo method, Type actionType, bool isbroadMatch = false)
            {
                if (message == null)
                ...
                lock (_map)
                {
                    if (isbroadMatch)
                    {
                        ...
                        foreach (string key in _map.Keys)
                        {
                            ...
                            {
                                _map[key].Add(new WeakAction(target, method, actionType));
                            }
                        }
                    }
                    else
